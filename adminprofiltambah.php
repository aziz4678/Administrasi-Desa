...
<body>
    ...
    $querydokter = $conn->query($sql);
    $sqll = "SELECT * FROM jadwal";
    $querydokterr = $conn->query($sqll);
    ?>
    ...
    <div class="d-flex bg-dark text-white" id="wrapper">
        ...
        <div id="page-content-wrapper">
            ...
            <div class="container-sm">
                ...
                <form action="adminprofiltambahaksi.php" method="post">
                    <div class="mb-3">
                        <label for="id_admin" class="form-label">Id Admin</label>
                        <input type="text" class="form-control" id="id_admin" name="id_admin">
                    </div>
                    <div class="mb-3">
                        <label for="inputiddokter" class="form-label">Nama Admin</label>
                        <select class="form-select" id="inputiddokter" name="id">
                            <option value="">Pilih Admin</option>
                            <?php
                            foreach ($querydokter as $datadokter) {
                            ?>
                                <option value="<?php echo $datadokter['id']; ?>"><?php echo $datadokter['nama'] ?></option>
                            <?php } ?>
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="inputiddokterr" class="form-label">Waktu Awal</label>
                        <select class="form-select" id="inputiddokterr" name="id_jadwal">
                            <option value="">Pilih Jadwal Awal</option>
                            <?php
                            foreach ($querydokterr as $datadokterr) {
                            ?>
                                <option value="<?php echo $datadokterr['id_jadwal']; ?>"><?php echo $datadokterr['waktu_mulai'] ?></option>
                            <?php } ?>
                        </select>
                    </div>
                    <!-- <div class="mb-3">
                        <label for="inputiddokterrr" class="form-label">Waktu Akhir</label>
                        <select class="form-select" id="inputiddokterrr" name="id_jadwal">
                            <option value="">Pilih Jadwal Akhir</option>
                            <--?php
                            foreach ($querydokterr as $datadokterrr) {
                            ?>
                                <option value="<--?php echo $datadokterr['id_jadwal']; ?>"><--?php echo $datadokterrr['waktu_akhir'] ?></option>
                            <-?php } ?>
                        </select>
                    </div>                        -->
